...
#     # 1️⃣ First, find colleges within the cutoff range
...
#     # 2️⃣ If fewer than 5 colleges, find the nearest cutoff match
...
#         # Remove used cutoff from available options to prevent duplicates
...
# with col1:
#     application_no = st.text_input("📄 Application Number", value="14545")  
#     name = st.text_input("👤 Full Name", value="Shyam Sundhar")  
#     email = st.text_input("📧 Email Address", value="shyamgsundhar@gmail.com")  
#     district = st.text_input("🏙️ District", value="Erode")  

# with col2:
#     phone = st.text_input("📞 Phone Number", value="9080435234")  
#     rank_no = st.number_input("🏅 Rank Number", min_value=1, step=1, value=1)  
#     cutoff_marks = st.number_input("📊 Enter Your Cutoff Mark", min_value=0.0, max_value=200.0, step=0.1, value=165.0)  

# # Department & Community Selection
# departments = show_options(database)
# selected_department = st.selectbox("🏛️ Select Department", departments, index=departments.index("BIO TECHNOLOGY") if "BIO TECHNOLOGY" in departments else 0)
...
#     selected_community = st.selectbox("🌍 Select Community", communities, index=communities.index("ST") if "ST" in communities else 0)
# else:
...
# if submit:
#     with st.spinner("🔄 LLM is Loading..."):
#         time.sleep(2)  
